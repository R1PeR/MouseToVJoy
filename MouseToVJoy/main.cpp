...
void UpdateCode() {
	...
	Sleep(2);
	if (ffbSize.EffectType() == "Constant") {
		if (ffbSize.Direction() > 100) {
			ffbStrength = (ffbSize.Magnitude())*(sw.ElapsedMilliseconds()*0.001);
		}		
		else {
			ffbStrength = -(ffbSize.Magnitude())*(sw.ElapsedMilliseconds()*0.001);
		}
	}
	if (ffbSize.EffectType() == "Period") {
		ffbStrength = (ffbSize.Offset()*0.5)*(sw.ElapsedMilliseconds()*0.001);
	}
	if (fR.result(21) == 1) {
		X = X + ffbStrength;
		ffbStrength = 0;
...
#pragma region Periodic
	...
	{
		ffbSize.EffectType("Period");
		ffbSize.Magnitude(EffPrd.Magnitude);
		ffbSize.Direction(Effect.DirX);
		ffbSize.Offset(TwosCompWord2Int(static_cast<WORD>(EffPrd.Offset)));
		ffbSize.Period(EffPrd.Period);
		ffbSize.Duration(Effect.Duration);
	};
#pragma endregion
...
#pragma region Constant Effect
	...
	{
		ffbSize.EffectType("Constant");
		ffbSize.Magnitude(ConstantEffect.Magnitude);
		ffbSize.Direction(Polar2Deg(Effect.Direction));
		ffbSize.Offset(0);
		ffbSize.Period(0);
		ffbSize.Duration(Effect.Duration);
	};
